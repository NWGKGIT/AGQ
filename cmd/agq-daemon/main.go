...
func run() error {
	...
	appState := state.New()
	addr := "127.0.0.1:" + envDefault("AGQ_PORT", "7432")
...
func ensureAGQDir() (string, error) {
	...
	if err := os.MkdirAll(agqDir, 0o700); err != nil {
		return "", fmt.Errorf("create %s: %w", agqDir, err)
	}
	if err := os.Chmod(agqDir, 0o700); err != nil {
		return "", fmt.Errorf("secure %s: %w", agqDir, err)
	}
	return agqDir, nil
}
